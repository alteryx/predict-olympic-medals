...
def select_multivalued_features(feature_matrix, features=None):
    '''
    Select features that have at least 2 unique values
    Args:
    ...
    keep = [c for c in feature_matrix
            if feature_matrix[c].nunique(dropna=False) > 1]
...
def select_non_null_features(feature_matrix, features=None):
    '''Select features that are not all null.
    Args:
        feature_matrix (:class:`pd.DataFrame`): DataFrame whose columns are
            feature names and rows are instances
        features (list[:class:`featuretools.PrimitiveBase`] or list[str], optional):
            List of features to select
    '''
    keep = [f for f in feature_matrix.columns
            if feature_matrix[f].dropna().shape[0] > 0]
    feature_matrix = feature_matrix[keep]
    if features is not None:
        features = [f for f in features
                    if f.get_name() in feature_matrix.columns]
